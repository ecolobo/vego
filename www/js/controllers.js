angular.module('starter.controllers', ['ionic', 'ngCordova'])
  ...
  .controller('AppCtrl', function ($scope, $state, $localStorage, $ionicPopup, $ionicHistory, Usuario, $http, API, $cordovaCamera,$ionicLoading) {
    $scope.voltar = function () {
      $ionicHistory.goBack();
    }

    $scope.lerPDF = function () {
      var pdf = {
        link: "http://www.culturaveg.com.br/wp-content/uploads/2018/01/Lista-Atualizada-do-Troll-Ajuda-06-01-2018-2.pdf",
        pagina: "8"
      }
      $ionicLoading.show({ animation: 'fade-in', showBackdrop: true, maxWidth: 200, showDelay: '0' });
      $http.post(API.url + '/api/v1/ler_pdf_url', pdf).then(function (data) {
        console.log(data);
        setTimeout(function () {
          $http.post(API.url + '/api/v1/ler_pdf_diretorio', pdf).then(function (sucesso) {
            console.log(sucesso);
            $ionicLoading.hide();
          })
        }, 10000);
  ...
  //.controller('LoginCtrl', function($scope, $state, $ionicHistory, $ionicLoading, $ionicModal, $ionicPopup, Usuario) {
  .controller('ConsultaPorNomeCtrl', function ($scope, $state, $rootScope, $state, $ionicHistory, $ionicLoading,
    $stateParams, $localStorage, $ionicLoading, $http, $ionicModal, $ionicPopup, API, Usuario, $cordovaBarcodeScanner, $cordovaCamera) {
    //aqui
    ...
    //dl inicio 27jun2018 - A lista abaixo mostra todos os produtos cadastrados pelos usuarios no VEGO
    $scope.pesquisar2 = function () {
      $ionicLoading.show({ animation: 'fade-in', showBackdrop: true, maxWidth: 200, showDelay: '0' });
      // aqui lista os produtos que apenas os veganators podem ver!
    ...
    //dl inicio 09 de setembro de 2018 - retorna os dados de 1 unica veganator
    $scope.pesquisarunicoveganator = function () {
      $ionicLoading.show({ animation: 'fade-in', showBackdrop: true, maxWidth: 200, showDelay: '0' });

      $http.post(API.url + 'api/v1/veganator_existente', { username: $localStorage.username, animal_codigo: $scope.camposPesquisa.animal_codigo, quantidade: API.quantidade }).then(function (sucesso) {
        $ionicLoading.hide();
        //    $rootScope.animais = sucesso.data.resultados;
        //      $rootScope.animais = sucesso.data;
        $rootScope.animais = sucesso.data.doc;
        ...
        // $rootScope.tipoRetorno = 'text';
        // $state.go("app.mostra_lista");
        $state.go("app.mostra_lista");// mesmo que abre o cadastro do veganator
      }, function (erro) { });
    };
    //dl fim 09 setembro de 2018
    ...
    //dl inicio  - casdastra o veganator
    $ionicModal.fromTemplateUrl('templates/modal-cadastro-veganators.html', {
      ...
      //aqui abaixo se conseguir inserir as variaveis do veganator sera possivel conseguir tb editar
      $scope.dadosCadastroMe = {
        produto: '',
        empresa: $localStorage.veganator,
        whatsapp: $localStorage.username,
        ingrediente: $localStorage.username
        , status: 'Pendente'
        ...
        if (formCadastroMe.$valid) {
          ...
          Usuario.cadastraveganators(dadosCadastroMe).then(function (response) {
            ...
            if (response.indexOf('inserido') > -1) {
              ...
              mensagemCadastro = "Você já esta cadastrado como VegaNator!";
            }
    ...
    $ionicModal.fromTemplateUrl('templates/modal-cadastro-produto.html', {
      ...
      function sendBase64(base64, type, maxResults) {
        return {
          "requests": [
            {
              "image": {
          ...
          if (data['responses'] != undefined) {
            if (data['responses'][0]['textAnnotations'] != undefined) {
              if (data['responses'][0]['textAnnotations'][0]['description'] != undefined) {
                $scope.dadosCadastro.ingrediente = data['responses'][0]['textAnnotations'][0]['description'];
              }
              else {
                alert('Nenhum texto encontrado, tente novamente.');
                this.loadingGoogle.dismiss();
              }
            }
            else {
              alert('Nenhum texto encontrado, tente novamente.');
      ...
      $scope.scanBarcode = function () {
        $cordovaBarcodeScanner.scan().then(function (imageData) {
          $scope.dadosCadastro.codigoDeBarras = imageData.text;
        }, function (err) {
          console.log(err)
        });
      }
      ...
      $scope.cadastrarUsuario = function (formCadastro, dadosCadastro) {
        if (formCadastro.$valid) {
          if (dadosCadastro.codigoDeBarras != '') {
            $ionicLoading.show({ animation: 'fade-in', showBackdrop: true, maxWidth: 200, showDelay: '0' });
  ...
  .controller('VisualizarAnimalCtrl', function ($scope, $rootScope, $state, $stateParams, $http, $localStorage, $ionicLoading, $ionicScrollDelegate, $cordovaNetwork, API) {
    ...
    if (!encontrado) {
      ...
      $http.post(API.url + 'api/v1/cga', {
        username: $localStorage.username,
        //cgaanimal: $stateParams.produto,
        //cgaanimal: parseInt($stateParams.cga),
        //cgaanimal: parseInt($stateParams.produto),
        cgaanimal: $stateParams.cga, //deu certo
        //cgaanimal: parseInt($stateParams.cga), //deu certo
        // cgaanimal: "dssd",
        // cgaanimal: API.cgaanimal,
        //cgaanimal: parseInt($localStorage.produto),
        //cga:  $localStorage.username,
        quantidade: API.quantidade
      }).then(function (sucesso) {
        console.log(sucesso, $localStorage.username);
        $ionicLoading.hide();

        //$scope.animal = sucesso.data.doc[0];
        // $scope.animal = sucesso.data.doc[0];
        $scope.animal = sucesso.data.doc;
        console.log(sucesso.data.doc.produto);
        //console.log($scope.animal.produto);
